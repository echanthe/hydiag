...
%    set(progfig,'CloseRequestFcn',@closeReq);
    set(progfig,'CloseRequestFcn',@closeBar); % don't ask, just quit
...
%it
if (etime(clock,lastupdate) < 0.05 && ~(percentdone == 100))
    return
end
...
if (fractiondone == 0)
    ...
    timeleftstr = sec2timestr(timeleft+1); % +1 to avoid 0s :)
